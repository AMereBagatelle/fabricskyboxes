        ...
        ClientTickEvents.END_CLIENT_TICK.register(client -> {
            while (toggleFabricSkyBoxes.wasPressed()) {
                ...
                if (SkyboxManager.getInstance().isEnabled()) {
                    client.player.sendMessage(new TranslatableText("fabricskyboxes.message.enabled"), false);
                } else {
                    client.player.sendMessage(new TranslatableText("fabricskyboxes.message.disabled"), false);
                }
            }
        });
    }
