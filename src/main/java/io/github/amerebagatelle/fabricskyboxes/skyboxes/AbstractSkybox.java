...
public abstract class AbstractSkybox implements FSBSkybox {
    ...
    protected boolean checkWorlds() {
        MinecraftClient client = MinecraftClient.getInstance();
        Objects.requireNonNull(client.world);
        return this.conditions.getWorlds().isEmpty() || this.conditions.getWorlds().contains(client.world.getDimension().getEffects());
    }
    ...
    protected boolean checkEffects() {
        ...
        if (this.conditions.getEffects().isEmpty()) {
            ...
            return !(camera.getFocusedEntity() instanceof LivingEntity livingEntity) || !livingEntity.hasStatusEffect(StatusEffects.BLINDNESS);
