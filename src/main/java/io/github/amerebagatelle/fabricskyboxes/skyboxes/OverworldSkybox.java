...
public class OverworldSkybox extends AbstractSkybox {
    ...
    public void render(WorldRendererAccess worldRendererAccess, MatrixStack matrices, float tickDelta) {
        ...
        if (SkyboxManager.getInstance().isEnabled() && SkyboxManager.getInstance().getActiveSkyboxes().stream().filter(skybox -> skybox instanceof AbstractSkybox).map(AbstractSkybox.class::cast).anyMatch(abstractSkybox -> abstractSkybox.getDecorations().getRotation().getSkyboxRotation())) {
            skyAngle = MathHelper.floorMod(world.getTimeOfDay() / 24000F + 0.75F, 1);
            skyAngleRadian = skyAngle * (float) (Math.PI * 2);
        }

        float[] fs = world.getSkyProperties().getFogColorOverride(skyAngle, tickDelta);
        if (fs != null) {
            RenderSystem.disableTexture();
