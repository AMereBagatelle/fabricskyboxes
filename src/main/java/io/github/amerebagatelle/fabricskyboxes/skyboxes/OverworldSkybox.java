...
public class OverworldSkybox extends AbstractSkybox {
    ...
    public void render(WorldRendererAccess worldRendererAccess, MatrixStack matrices, Matrix4f projectionMatrix, float tickDelta, Camera camera, boolean thickFog) {
        ...
        worldRendererAccess.getLightSkyBuffer().setShader(matrices.peek().getPositionMatrix(), projectionMatrix, shaderProgram);
        VertexBuffer.unbind();
        ...
        if (d < 0.0) {
            ...
            worldRendererAccess.getDarkSkyBuffer().setShader(matrices.peek().getPositionMatrix(), projectionMatrix, shaderProgram);
            matrices.pop();
        }
