...
public class AnimatableTexture {
    ...
    public void update(long gameTime, float tickDelta) {
        if (this.frameCount == 1 || this.animationDuration <= 0L) {
        ...
        for (int frameIndex = 0; frameIndex < this.frameCount; frameIndex++) {
            ...
            if (cycleTime < frameEnd || frameIndex == this.frameCount - 1) {
                float blend = this.interpolate
                        ? (float) Mth.clamp((cycleTime - frameStart) / (double) frameDuration, 0.0D, 1.0D)
                        : 0.0F;
                this.setCurrentFrame(frameIndex, blend);
                return;
            }
            frameStart = frameEnd;
        }
    ...
    private void setCurrentFrame(int frameIndex, float blend) {
        if (this.currentFrame == null || this.index != frameIndex) {
            ...
            this.nextFrame = this.calculateFrameUVRange((frameIndex + 1) % this.frameCount);
        }
        this.frameBlend = blend;
    }
    ...
    private static long calculateAnimationDurationMillis(
            int frameCount,
            long defaultDuration,
            Map<Integer, Long> frameDuration
    ) {
